...
class pv {
    constructor(id, title, description, time, author, visitable, tags) {
        ...
        this.tags = tags;
        this.html = `<div class="card" style="height:225px;max-width:666px;"><img src="/resource/post/avatar/${this.id.toString().padStart(8, "0")}.png" alt="${this.title}"><div class="card-text"><h2>${this.title}</h2><p><b>${this.description}</b></p><p style="color:grey">作者: ${this.author}</p><dl class="card-tag">${this.tags.map(tag => `<dd>${tag}</dd>`).join('')}<dd>${new Date(this.time).toLocaleDateString('zh-CN')} ${new Date(this.time).toLocaleTimeString('zh-CN')}</dd></dl></div></div>`;
    }
}
